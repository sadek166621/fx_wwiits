...
use App\Models\DepositGift;
use App\Models\DepositProfit;
use App\Models\DepositReturnHistory;
...
class PagesController extends Controller
{
    ...
    public function updateMember()
    {
        ...
        foreach ($members as $member){
            $deposit_member = Deposit::where('member_id', $member->id)->get();
            $total_profit = 0;
                    ...
                    foreach($deposit_member as $deposit){
                        ...
                        if($differenceInDays > 0 && $deposit->amount > 0 && $differenceInDays <= $deposit->package->maturity_time){
                            if($deposit->status == 1){
                                ...
                                $total_profit += $deposit->profit_amount;
                            }
                            ...
                            foreach ($deposit_returns as $deposit_return){
                                ...
                                if ($differenceInDays == $deposit_days){
                                    ...
                                    if($returning_amount > 0){
                                        DepositReturnHistory::create([
                                            'deposit_id' => $deposit->id,
                                            'previous_amount' =>$deposit->remaining_balance ?? $deposit->amount,
                                            'returning_amount' => $returning_amount,
                                            'current_amount' => $deposit->remaining_balance - $returning_amount
                                        ]);
                                    }
                            ...
                            if($deposit->remaining_balance > 0 ){
...
//                            $deposit->delete();
                    ...
                    $member->update([
                        'profit'=> $member->profit+$total_profit,
                    ]);
                }
                ...
                $profit = $total_profit;
//                return view('frontend.email.daily-update', ['member'=>$member, 'profit'=>$profit]);
...
//                        return redirect()->route('profile-settings');
    ...
    public function studentprofileupdate(Request $request, $id){
        ...
        if($student){
            ...
            $voter_id_back = $student->voter_id_card_back;
            $image = $request->file('voter_id_card_back');
            if($image){
                $currentDate = Carbon::now()->toDateString();
                //dd($image->getClientOriginalExtension());
            ...
            $student->update([
                ...
                'voter_id_card_back' => $voter_id_back,
    ...
    public function withdraw(){
        $data['passbook'] = Passbook::where('student_id',Session::get('StudentId'))->get();
        $data['student'] = Student::where('id', Session::get('StudentId'))->first();
        $data['packages'] = Deposit::where('member_id', Session::get('StudentId'))->where('remaining_balance', '>', 0)->get();
...
//            return $request;
            ...
            $withdraw_amount = $amount->remaining_balance;
...
//          return $data ;
    ...
    public function DepositReturnHistory()
    {
        $member = Student::where('id',Session::get('StudentId'))->first();
        $deposits = Deposit::where('member_id', $member->id)->pluck('id')->toArray();
        $data['student'] = $member;
        $data['items'] = DepositReturnHistory::whereIn('deposit_id', $deposits)->get();
        return view('frontend.deposit_return_history', $data);
    }
    public function DepositGiftHistory()
    {
        $member = Student::where('id',Session::get('StudentId'))->first();

        $data['student'] = $member;
        $data['items'] = DepositGift::where('member_id', $member->id)->get();
        return view('frontend.deposit_gift_history', $data);
    }
